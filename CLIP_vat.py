...
class ImageAudioModel(torch.nn.Module):
    ...
    def forward(self, images, audios, text, attention_mask):
        ...
        audio_features = audio_features[:,0,:].unsqueeze(1)  # [batch_size, 1, 768]
...
def main():
    ...
    for epoch in range(10):
        for images, audios, text, attention_mask, labels in dataloader:
            ...
            optimizer.zero_grad()
            image_features, audio_features, text_features = model(images, audios, text, attention_mask)
            image_text_loss = criterion(image_features, text_features)
            audio_text_loss = criterion(audio_features, text_features)
            loss = image_text_loss + audio_text_loss
            loss.backward()
            optimizer.step()
